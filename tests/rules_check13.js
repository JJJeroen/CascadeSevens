...
check(
  "brutalcritic finding 1a: swapJoker refuses to run when it would strand the joker as an unmeldable last card",
  () => {
    ...
    if (!threw)
      throw new Error(
        "swap should have been refused -- it would strand the joker as an unmeldable last card",
      );
    if (
      game.round.hands[0].length !== 1 ||
      game.round.hands[0][0].id !== "7S"
    ) {
      throw new Error("a refused swap must not mutate hand/tableau at all");
    }
    if (game.round.pendingObligations.length !== 0)
      throw new Error("a refused swap must not leave a dangling obligation");
  },
);

check(
  "brutalcritic finding 1a (control): the same swap succeeds with one spare card in hand",
  () => {
    const game = freshGameAtPart2();
    game.round.tableau.push({
      id: "m1",
    ...
    if (game.round.pendingObligations.length !== 1)
      throw new Error("joker should now be a pending obligation");
    ...
    if (game.round.pendingObligations.length !== 0)
      throw new Error("obligation should now be cleared");
    if (game.round.hands[0].length !== 1)
      throw new Error("exactly the spare card should remain");
  },
);

check(
  "brutalcritic finding 1b: addToMeld refuses to strand an untouched row-take obligation",
  () => {
    const game = freshGameAtPart2();
    ...
    game.round.hands[0][0] = card("K", "C");
    game.round.hands[0][0].id = "XC";
    game.round.tableau.push({
      id: "m1",
      type: "set",
      slots: [
    ...
    let threw = false;
    try {
      E.addToMeld(game, "m1", "5D");
    } catch (e) {
      threw = true;
    }
    if (!threw)
      throw new Error(
        "should have refused -- this would strand the obligated card XC as the sole remaining card",
      );
    if (game.round.hands[0].length !== 2)
      throw new Error("a refused add must not mutate hand at all");
  },
);

check(
  "brutalcritic finding 1b (control): adding the obligated card itself, leaving the spare, still works",
  () => {
    const game = freshGameAtPart2();
    ...
    if (game.round.hands[0].length !== 1 || game.round.hands[0][0].id !== "5D")
      throw new Error("the spare card should remain, ready to discard");
    E.discard(game, "5D");
    if (!game.round.ended)
      throw new Error("round should end via the natural last-card discard");
  },
);

check(
  "brutalcritic finding 1: layNewMeld refuses a selection that would strand an unselected obligation",
  () => {
    const game = freshGameAtPart2();
    ...
    if (!threw)
      throw new Error(
        "should have refused -- this strands the obligated 9C as the sole remaining card",
      );
    if (game.round.hands[0].length !== 4)
      throw new Error("a refused layNewMeld must not mutate hand at all");
  },
);
