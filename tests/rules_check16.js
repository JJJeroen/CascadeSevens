...
function freshGameAtPart1() {
  ...
  E.turn0Decline(game);
  E.turn0Decline(game); // lands on part === 1, current === 0
  return game;
}

check(
  "pile-empty: both players penalized for their own hand, neither gets the winner bonus",
  () => {
    ...
    if (!r.ended || r.endReason !== "pile-empty")
      throw new Error("round should end via pile-empty");
    if (r.roundWinner !== null)
      throw new Error("pile-empty has no winner, got " + r.roundWinner);
    if (r.roundScores[0] !== -5)
      throw new Error(
        "player 0 should be penalized exactly -5, got " + r.roundScores[0],
      );
    if (r.roundScores[1] !== -10)
      throw new Error(
        "player 1 should be penalized exactly -10, got " + r.roundScores[1],
      );
  },
);

check(
  "pile-empty: meld points already on the table still count for their owner, alongside the penalty",
  () => {
    const game = freshGameAtPart1();
    const r = game.round;
    r.closedPile = [];
    ...
    if (r.roundScores[0] !== 10)
      throw new Error(
        "player 0: 15 melds - 5 penalty = 10, got " + r.roundScores[0],
      );
    if (r.roundScores[1] !== -5)
      throw new Error(
        "player 1: 0 melds - 5 penalty = -5, got " + r.roundScores[1],
      );
  },
);
...
check(
  ...
  () => {
    ...
    if (r.hands[0].length !== 2)
      throw new Error("the drawn card should have been added to hand");
  },
);
