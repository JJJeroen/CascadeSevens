...
check(
  "canStartRearrange requires come-out, Part 2, and no pending obligation",
  () => {
    const game = E.newGame("standard", () => 0.1);
    E.startRound(game, () => 0.5);
    E.turn0Decline(game);
...
check("rearrange: split one meld into two valid ones", () => {
  ...
  if (commitFail.ok)
    throw new Error("should have failed: both groups have only 2 cards");
  ...
  if (!commitOk.ok)
    throw new Error(
      "should succeed once merged back: " + JSON.stringify(commitOk.problems),
    );
  if (
    game.round.tableau.length !== 1 ||
    game.round.tableau[0].slots.length !== 4
  ) {
...
check(
  "rearrange: reorganize two melds into different valid groupings, plus a hand card",
  () => {
    ...
    if (!commit.ok)
      throw new Error(
        "no-op rearrange should trivially succeed: " +
          JSON.stringify(commit.problems),
      );
    const runMeld = game.round.tableau.find((m) =>
      m.slots.some((s) => s.card.id === "5D"),
    );
    const setMeld = game.round.tableau.find((m) =>
      m.slots.some((s) => s.card.id === "KS"),
    );
    if (runMeld.slots.length !== 3 || setMeld.slots.length !== 3)
      throw new Error("meld sizes changed on a no-op commit");
    if (runMeld.slots.find((s) => s.card.id === "7D").ownerId !== 0)
      throw new Error("ownership should be unchanged for untouched cards");
    if (setMeld.slots.find((s) => s.card.id === "KS").ownerId !== 1)
      throw new Error(
        "opponent ownership should be unchanged for untouched cards",
      );
  },
);

check(
  "rearrange: pre-existing card keeps ORIGINAL owner even when moved to a different/new group",
  () => {
    ...
    if (commitShouldFail.ok)
      throw new Error("should fail: 5D,6D left as an invalid 2-card remainder");
    ...
    const remainderGid = st2.groups.find(
      (g) => g.groupId !== newGid2 && g.cardIds.length > 0,
    ).groupId;
    E.rearrangeMoveCard(game, "4D", remainderGid);
    const commit = E.commitRearrange(game);
    if (!commit.ok)
      throw new Error("expected success: " + JSON.stringify(commit.problems));
    const newSet = game.round.tableau.find((m) =>
      m.slots.some((s) => s.card.id === "7H"),
    );
    const sevenD = newSet.slots.find((s) => s.card.id === "7D");
    const sevenH = newSet.slots.find((s) => s.card.id === "7H");
    if (sevenD.ownerId !== 1)
      throw new Error(
        "pre-existing 7D should keep its ORIGINAL owner (1), got " +
          sevenD.ownerId,
      );
    if (sevenH.ownerId !== 0)
      throw new Error(
        "newly-added 7H (from hand) should be owned by the committing player (0)",
      );
  },
);

check(
  "rearrange: cannot leave an opponent-owned card sitting in hand at commit",
  () => {
    ...
    const commit = E.commitRearrange(game);
    if (commit.ok)
      throw new Error("should reject leaving an opponent card in hand");
    if (
      !commit.problems.some((p) =>
        p.error.includes("belongs to the other player"),
      )
    ) {
      throw new Error(
        "expected the specific ownership problem message, got " +
          JSON.stringify(commit.problems),
      );
    }
  },
...
check(
  "rearrange: CAN leave your OWN pre-existing card sitting in hand at commit",
  () => {
    const game = freshGameAtPart2([card("2", "H")]);
    game.round.tableau.push({
      id: "m1",
    ...
    const commit = E.commitRearrange(game);
    if (!commit.ok)
      throw new Error("should succeed: " + JSON.stringify(commit.problems));
    if (game.round.tableau.length !== 0)
      throw new Error("tableau should be empty");
    if (game.round.hands[0].length !== 4)
      throw new Error("hand should have all 4 cards (2H + the 3 nines)");
  },
);
...
check("rearrange: cannot commit with zero cards left in hand", () => {
  ...
  if (!commit.problems.some((p) => p.error.includes("at least one card"))) {
    throw new Error(
      "expected the empty-hand problem message, got " +
        JSON.stringify(commit.problems),
    );
  }
});
...
check("cancelRearrange fully reverts to the exact pre-session state", () => {
  ...
  E.cancelRearrange(game);
  if (JSON.stringify(game.round.tableau) !== tableauBefore)
    throw new Error("tableau not restored exactly");
  if (JSON.stringify(game.round.hands[0]) !== handBefore)
    throw new Error("hand not restored exactly");
  if (game.round.rearrange !== null)
    throw new Error("session should be cleared");
});

check(
  "rearrangeState reports live valid/invalid per group for UI feedback",
  () => {
    const game = freshGameAtPart2([card("2", "H")]);
    game.round.tableau.push({
      id: "m1",
    ...
    E.startRearrange(game);
    const st = E.rearrangeState(game);
    if (!st.groups[0].valid)
      throw new Error("untouched 3-card set should read as valid");
    ...
    if (remainder.valid)
      throw new Error("2-card remainder should read as invalid");
    if (newGroup.valid)
      throw new Error("lone 1-card group should read as invalid");
  },
);
