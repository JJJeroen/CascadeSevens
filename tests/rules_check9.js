...
check("liveScore does not double-count after the round has ended", () => {
  ...
  if (liveP0 !== game.scores[0])
    throw new Error(
      `liveScore(0)=${liveP0} should equal game.scores[0]=${game.scores[0]} once round ended`,
    );
  if (liveP1 !== game.scores[1])
    throw new Error(
      `liveScore(1)=${liveP1} should equal game.scores[1]=${game.scores[1]} once round ended`,
    );
});
