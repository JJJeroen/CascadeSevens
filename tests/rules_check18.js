...
check(
  "exactly at the threshold (=1000) does not end the game -- must be STRICTLY greater",
  () => {
    const game = freshGameAtPart2();
    game.scores = [950, 700];
    endHandoutRound(game, card("2", "C"), card("2", "H")); // winner +50, loser -5
    if (game.scores[0] !== 1000)
      throw new Error("expected winner score 1000, got " + game.scores[0]);
    if (game.gameOver)
      throw new Error("game must not end at exactly the threshold");
  },
);
...
check("crossing strictly above the threshold while ahead ends the game", () => {
  ...
  if (game.scores[0] !== 1001)
    throw new Error("expected 1001, got " + game.scores[0]);
  if (!game.gameOver || game.winner !== 0)
    throw new Error(
      "game should end with player 0 winning once strictly over threshold and ahead",
    );
});
...
check("both players over the threshold but tied does not end the game", () => {
  ...
  if (game.gameOver)
    throw new Error(
      "a tie above the threshold must not end the game -- neither player is STRICTLY ahead",
    );
});

check(
  "being ahead while still under the threshold does not end the game",
  () => {
    const game = freshGameAtPart2();
    game.scores = [900, 500];
    ...
    if (game.gameOver)
      throw new Error(
        "being ahead is not enough without also crossing the threshold",
      );
  },
);
...
check("quick mode ends the game at its own 300 threshold, not 1000", () => {
  ...
  if (game.threshold !== 300)
    throw new Error(
      "quick mode should set threshold=300, got " + game.threshold,
    );
  game.scores = [296, 100];
  endHandoutRound(game, card("2", "C"), card("2", "H"));
  if (game.scores[0] !== 346)
    throw new Error("expected 346, got " + game.scores[0]);
  if (!game.gameOver || game.winner !== 0)
    throw new Error(
      "quick mode should end once strictly over its own 300 threshold and ahead",
    );
});
...
check("quick mode: exactly at 300 does not end the game", () => {
  ...
  if (game.scores[0] !== 300)
    throw new Error("expected exactly 300, got " + game.scores[0]);
  if (game.gameOver)
    throw new Error(
      "quick mode must not end at exactly its own threshold either",
    );
});
