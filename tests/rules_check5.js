...
check(
  "come-out progress carries forward across turns (not reset each turn)",
  () => {
    ...
    if (game.round.current !== 0)
      throw new Error("test setup: expected it to be player 0 turn again");
    E.drawFromClosedPile(game);
    ...
    if (!game.round.comeOut[0])
      throw new Error(
        "should have come out: 30 (earlier turn) + 30 (this turn) = 60 >= 40",
      );
  },
);
