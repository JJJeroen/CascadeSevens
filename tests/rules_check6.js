...
check(
  "addToMeld: extending the LOW end prepends, not appends (display order)",
  () => {
    ...
    if (order.join(",") !== "9,10,J,Q")
      throw new Error("expected 9,10,J,Q order, got " + order.join(","));
  },
);
check("addToMeld: extending the HIGH end still appends correctly", () => {
  ...
  const order = game.round.tableau[0].slots.map((s) => s.card.rank);
  if (order.join(",") !== "10,J,Q,K")
    throw new Error("expected 10,J,Q,K order, got " + order.join(","));
});
check(
  "addToMeld: low-end extension keeps the meld valid for further extension",
  () => {
    const game = freshGameAtPart1();
    E.drawFromClosedPile(game);
    game.round.comeOut[game.round.current] = true;
    ...
    const order = game.round.tableau[0].slots.map((s) => s.card.rank);
    if (order.join(",") !== "8,9,10,J,Q")
      throw new Error("expected 8,9,10,J,Q order, got " + order.join(","));
  },
);
