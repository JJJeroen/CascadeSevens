...
check(
  "addToMeld: joker repositions to make room for a non-adjacent card (the exact reported scenario)",
  () => {
    ...
    const order = meld.slots.map((s) =>
      s.card.rank === "JOKER" ? `J->${s.wildAs.rank}` : s.card.rank,
    );
    if (order.join(",") !== "10,J,J->Q,K")
      throw new Error("expected 10,J,J->Q,K, got " + order.join(","));
    const jokerSlot = meld.slots.find((s) => s.card.rank === "JOKER");
    if (jokerSlot.ownerId !== 0)
      throw new Error(
        "joker should keep its original owner (0), got " + jokerSlot.ownerId,
      );
    const kSlot = meld.slots.find((s) => s.card.id === "KS");
    if (kSlot.ownerId !== game.round.current)
      throw new Error("the newly added K should be owned by the adding player");
    const tenSlot = meld.slots.find((s) => s.card.id === "10S");
    if (tenSlot.ownerId !== 1)
      throw new Error(
        "pre-existing real card should keep its original owner (1)",
      );
  },
);
...
check("addToMeld: still rejects when no repositioning makes it valid", () => {
  ...
  let threw = false;
  try {
    E.addToMeld(game, "m-run2", "KH");
  } catch (e) {
    threw = true;
  }
  if (!threw)
    throw new Error(
      "should still reject an unreachable extension with no joker to help",
    );
});

check(
  "addToMeld: joker at the low end slides to fill a gap opened by the new high card",
  () => {
    const game = freshGameAtPart1();
    E.drawFromClosedPile(game);
    game.round.comeOut[game.round.current] = true;
    ...
    if (order.join(",") !== "3,4,J->5,6")
      throw new Error("expected 3,4,J->5,6, got " + order.join(","));
  },
);
