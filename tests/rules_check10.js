...
check(
  "AI finds a valuable pickup buried deep in the row, not just position 0",
  () => {
    ...
    if (draw.source !== "row")
      throw new Error(
        "AI should have taken from the row, took: " + draw.source,
      );
    if (draw.cardId !== "3D")
      throw new Error(
        "AI should have targeted 3D (the useful buried run), took: " +
          draw.cardId,
      );
  },
);
...
check(
  "AI still declines when nothing in the row is actually resolvable",
  () => {
    const game = E.newGame("standard", () => 0.1);
    E.startRound(game, () => 0.5);
    E.turn0Decline(game);
    ...
    const draw = CascadeAI.pickDraw(game);
    if (draw.source !== "closed")
      throw new Error(
        "AI should have drawn from the closed pile, took: " + draw.source,
      );
  },
);
