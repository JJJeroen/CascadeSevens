...
check(
  "row draw can repeat; closed pile locks out after the first row draw",
  () => {
    ...
    if (!threw)
      throw new Error("drawFromClosedPile should have thrown once locked out");
    // repeat: take again
    if (!E.canDrawFromRow(game))
      throw new Error("row should still be drawable");
    const handLenBefore = game.round.hands[0].length;
    E.drawFromOpenRow(game, "4S");
    if (game.round.hands[0].length !== handLenBefore + 1)
      throw new Error("second scoop should add exactly 1 card to hand");
  },
);
...
check("only the most recent row draw is the binding obligation", () => {
  ...
  if (JSON.stringify(game.round.pendingObligations) !== JSON.stringify(["5S"]))
    throw new Error("first obligation wrong");
  E.drawFromOpenRow(game, "4S");
  if (JSON.stringify(game.round.pendingObligations) !== JSON.stringify(["4S"]))
    throw new Error(
      "obligation should be replaced, not accumulated: " +
        JSON.stringify(game.round.pendingObligations),
    );
});
...
check(
  ...
  () => {
    ...
    E.drawFromOpenRow(game, "9H");
    if (E.canDrawFromClosedPile(game))
      throw new Error("should be locked out right after the draw");
    E.undoDraw(game);
    if (!E.canDrawFromClosedPile(game))
      throw new Error("undo should restore closed-pile availability");
  },
);

check(
  "undo after a repeat draw restores the PREVIOUS obligation, not none",
  () => {
    const game = freshGameAtPart1([
      card("2", "S"),
      card("3", "S"),
    ...
    if (
      JSON.stringify(game.round.pendingObligations) !== JSON.stringify(["5S"])
    ) {
      throw new Error(
        "expected obligation to revert to 5S, got " +
          JSON.stringify(game.round.pendingObligations),
      );
    }
  },
...
check("autoResolveMeld: pure set, no jokers", () => {
  ...
  if (!r.ok || r.type !== "set")
    throw new Error("expected a set: " + JSON.stringify(r));
});
check("autoResolveMeld: set with a joker, rank auto-assigned", () => {
  ...
  if (jokerSlot.wildAs.rank !== "7")
    throw new Error("joker should represent rank 7");
});
check("autoResolveMeld: run with a joker filling an internal gap", () => {
  ...
  if (!r.ok || r.type !== "run")
    throw new Error("expected a run: " + JSON.stringify(r));
  const jokerSlot = r.slots.find((s) => s.cardId === "JOKER-1");
  if (jokerSlot.wildAs.rank !== "7")
    throw new Error(
      "joker should fill the gap at 7, got " + jokerSlot.wildAs.rank,
    );
});
check("autoResolveMeld: run with 2 jokers extending beyond the reals", () => {
  const hand = [
    ...
    card("6", "S"),
    { id: "JOKER-1", rank: "JOKER", suit: null },
    { id: "JOKER-2", rank: "JOKER", suit: null },
  ];
  ...
  if (!check2.ok)
    throw new Error(
      "autoResolveMeld produced a selection the real validator rejects: " +
        check2.error,
    );
});
check("autoResolveMeld: Q-K-A ace-high anchor via a joker", () => {
  ...
  if (!check2.ok)
    throw new Error(
      "produced selection rejected by real validator: " + check2.error,
    );
});
check(
  "autoResolveMeld: no wraparound (K,A,2 impossible even with help)",
  () => {
    const hand = [card("K", "S"), card("A", "S"), card("2", "S")];
    const r = E.autoResolveMeld(hand, ["KS", "AS", "2S"]);
    if (r.ok)
      throw new Error(
        "K-A-2 wraparound should never resolve, got " + JSON.stringify(r),
      );
  },
);
...
check("autoResolveAddToMeld: joker into a set uses the set rank", () => {
  ...
  const r = E.autoResolveAddToMeld(meld, {
    id: "JOKER-1",
    rank: "JOKER",
    suit: null,
  });
  if (!r || r.wildAs.rank !== "9")
    throw new Error("expected rank 9: " + JSON.stringify(r));
});
check("autoResolveAddToMeld: joker into a run picks a legal end", () => {
  ...
  if (!r || (r.wildAs.rank !== "4" && r.wildAs.rank !== "8"))
    throw new Error("expected an end extension: " + JSON.stringify(r));
});
check("autoResolveAddToMeld: non-joker card needs no wildAs", () => {
  const meld = {
    type: "set",
    slots: [{ card: card("9", "S"), ownerId: 0, wildAs: null }],
  };
  const r = E.autoResolveAddToMeld(meld, card("9", "H"));
  if (!r || r.wildAs !== undefined)
    throw new Error("real card should not get a wildAs: " + JSON.stringify(r));
});
