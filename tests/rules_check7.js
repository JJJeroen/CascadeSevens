...
check(
  "AI prefers swapping a joker over just adding, when replay is safe",
  () => {
    ...
    if (jokerStillThere)
      throw new Error(
        "AI added 7C alongside the joker instead of swapping it out",
      );
  },
);

check(
  "AI falls back to a plain add when the joker would not be safely replayable",
  () => {
    const game = E.newGame("standard", () => 0.1);
    E.startRound(game, () => 0.5);
    E.turn0Decline(game);
    ...
    if (!jokerStillThere)
      throw new Error(
        "AI swapped the joker out even though it could not have safely replayed it",
      );
  },
);

check(
  "canResolvePickup: joker filling an INTERNAL gap between two non-adjacent reals (the exact reported bug: Q _ A needing a joker as K)",
  () => {
    ...
    if (!CascadeAI.canResolvePickup(hand, openRow, "QH")) {
      throw new Error(
        "should find the Q,JOKER(as K),A-hearts run and report the pickup as resolvable",
      );
    }
  },
);
...
check(
  "canResolvePickup: still correctly rejects when no meld is actually possible",
  () => {
    const hand = [
      card("3", "C"),
      card("5", "H"),
    ...
    if (CascadeAI.canResolvePickup(hand, openRow, "7C")) {
      throw new Error(
        "should NOT find any meld for a genuinely unmeldable card",
      );
    }
  },
);
...
check(
  "canResolvePickup: a joker extends an adjacent pair downward (10-joker,J,Q), not just upward",
  () => {
    ...
    if (!CascadeAI.canResolvePickup(hand, openRow, "JOKER-1")) {
      throw new Error(
        "should find 10(joker),J,Q as a valid run using downward extension",
      );
    }
  },
);
