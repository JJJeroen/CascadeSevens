...
check(
  "turn0: P1 (starter) accepts immediately -> no follow-up, Turn 1 goes to P2",
  () => {
    const game = E.newGame("standard", () => 0.1); // starter = 0
    E.startRound(game, () => 0.5);
    const swapCard = game.round.hands[0][0].id;
    E.turn0Accept(game, swapCard);
    if (E.turn0CurrentAskee(game) !== null)
      throw new Error("P2 still being offered a follow-up");
    if (game.round.part !== 1 || game.round.current !== 1)
      throw new Error(
        "Turn 1 should go to P2 (the non-acceptor), got current=" +
          game.round.current,
      );
  },
);
check("turn0: P1 declines, P2 accepts -> P1 gets a follow-up", () => {
  ...
  E.startRound(game, () => 0.5);
  E.turn0Decline(game);
  if (E.turn0CurrentAskee(game) !== 1)
    throw new Error("P2 not offered after P1 declined");
  const swapCard = game.round.hands[1][0].id;
  E.turn0Accept(game, swapCard);
  if (E.turn0CurrentAskee(game) !== 0)
    throw new Error("P1 not offered the consolation follow-up");
});
check(
  "turn0: P1 declines, P2 accepts, P1 declines followup -> Turn 1 goes to P1",
  () => {
    const game = E.newGame("standard", () => 0.1);
    E.startRound(game, () => 0.5);
    E.turn0Decline(game);
    ...
    if (game.round.current !== 0)
      throw new Error(
        "Turn 1 should go to P1 (opponent of the last acceptor, P2), got " +
          game.round.current,
      );
  },
);
check(
  "turn0: P1 declines, P2 accepts, P1 ALSO accepts the followup -> Turn 1 goes to P2",
  () => {
    const game = E.newGame("standard", () => 0.1);
    E.startRound(game, () => 0.5);
    E.turn0Decline(game);
    ...
    if (game.round.current !== 1)
      throw new Error(
        "Turn 1 should go to P2 (opponent of the last acceptor, P1), got " +
          game.round.current,
      );
  },
);
check(
  "turn0: P1 declines, P2 declines -> no trigger, row length 1, starter begins Turn 1",
  () => {
    const game = E.newGame("standard", () => 0.1);
    E.startRound(game, () => 0.5);
    E.turn0Decline(game);
    E.turn0Decline(game);
    if (game.round.openRow.length !== 1)
      throw new Error("row length " + game.round.openRow.length);
    if (game.round.part !== 1) throw new Error("did not begin normal rotation");
    if (game.round.current !== 0)
      throw new Error(
        "nobody accepted anything -- the original starter should begin Turn 1, got " +
          game.round.current,
      );
  },
);
...
check("layNewMeld rejects using the entire hand", () => {
  ...
  try {
    E.layNewMeld(
      game,
      hand.map((c) => ({ cardId: c.id })),
    );
  } catch (e) {
    threw = true;
  }
  if (!threw)
    throw new Error("was allowed to lay entire 3-card hand as a meld");
});
...
check("pullFromMeld: removing a run edge card is legal", () => {
  ...
  if (meld.slots.length !== 3)
    throw new Error("expected 3 remaining, got " + meld.slots.length);
  const hand = game.round.hands[game.round.current];
  if (!hand.some((c) => c.id === "7S"))
    throw new Error("pulled card not returned to hand");
});
check("pullFromMeld: removing a run middle card is illegal", () => {
  ...
  let msg = "";
  try {
    E.pullFromMeld(game, "m-run2", "5S");
  } catch (e) {
    threw = true;
    msg = e.message;
  }
  if (!threw)
    throw new Error("was allowed to split a run by pulling a middle card");
  if (msg.includes("credited to the other player"))
    throw new Error(
      "rejected for the wrong reason (ownership, not the middle-split rule): " +
        msg,
    );
});
...
check("pullFromMeld: atomic multi-card pull fully dissolves a meld", () => {
  ...
  try {
    E.pullFromMeld(game, "m-run4", "6S");
  } catch (e) {
    singleThrew = true;
  }
  if (!singleThrew)
    throw new Error(
      "single-card pull from a 3-card meld should have been rejected",
    );
  E.pullFromMeld(game, "m-run4", ["4S", "5S", "6S"]);
  if (game.round.tableau.some((m) => m.id === "m-run4"))
    throw new Error("meld should have been removed from the tableau");
  const hand = game.round.hands[game.round.current];
  if (!["4S", "5S", "6S"].every((id) => hand.some((c) => c.id === id)))
    throw new Error("all 3 cards should be back in hand");
});
...
check("pullFromMeld rejects pulling a card owned by the other player", () => {
  ...
  let msg = "";
  try {
    E.pullFromMeld(game, "m-mixed", "KS");
  } catch (e) {
    threw = true;
    msg = e.message;
  }
  if (!threw)
    throw new Error("was allowed to pull a card owned by the other player");
  if (!msg.includes("credited to the other player"))
    throw new Error("wrong rejection reason: " + msg);
  ...
  const hand = game.round.hands[game.round.current];
  if (!hand.some((c) => c.id === "KD"))
    throw new Error("own card from a mixed meld should have been pullable");
});
check("re-laying a pulled (own) card keeps the same owner", () => {
  ...
  const meldNow = game.round.tableau.find((m) =>
    m.slots.some((s) => s.card.id === "KS"),
  );
  const slot = meldNow.slots.find((s) => s.card.id === "KS");
  if (slot.ownerId !== game.round.current)
    throw new Error(
      "re-laid card should still be owned by the same (only possible) player",
    );
});
