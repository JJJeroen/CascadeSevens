...
check(
  "discard: cannot discard a DIFFERENT card while the row obligation is still outstanding",
  () => {
    const game = freshGameAtPart1();
    game.round.openRow = [card("9", "H")];
    E.drawFromOpenRow(game, "9H");
    ...
    let threw = false;
    try {
      E.discard(game, otherCardId);
    } catch (e) {
      threw = true;
    }
    if (!threw)
      throw new Error(
        "should refuse to discard anything other than the outstanding obligated card",
      );
  },
);

check(
  "discard: a joker swap-out obligation may NOT be discarded back -- still must be melded",
  () => {
    const game = freshGameAtPart1();
    ...
    game.round.tableau.push({
      ...
      slots: [
        {
          card: { id: "JOKER-1", rank: "JOKER", suit: null },
          ownerId: 1,
          wildAs: { rank: "7" },
        },
    ...
    if (game.round.pendingObligations[0] !== "JOKER-1")
      throw new Error("joker should be the pending obligation");
    if (game.round.rowObligationCardId !== null)
      throw new Error("a swap obligation must not be marked discard-eligible");
    let threw = false;
    try {
      E.discard(game, "JOKER-1");
    } catch (e) {
      threw = true;
    }
    if (!threw)
      throw new Error(
        "should refuse to discard the reclaimed joker -- it must be melded",
      );
  },
);
...
check(
  "undoDraw restores rowObligationCardId correctly (repeated row-takes within one Part 1)",
  () => {
    const game = freshGameAtPart1();
    game.round.openRow = [card("3", "C"), card("9", "H")];
    E.drawFromOpenRow(game, "9H"); // first take, obligation = 9H
    if (game.round.rowObligationCardId !== "9H")
      throw new Error("setup: expected 9H obligation");
    E.undoDraw(game);
    if (game.round.rowObligationCardId !== null)
      throw new Error(
        "undo should restore rowObligationCardId to null (nothing was obligated before)",
      );
    if (game.round.pendingObligations.length !== 0)
      throw new Error("undo should restore pendingObligations to empty");
  },
);
...
check(
  ...
  () => {
    ...
    if (game.round.rowObligationCardId !== "3C")
      throw new Error("rowObligationCardId should now be 3C, not 9H");
    if (
      game.round.pendingObligations.length !== 1 ||
      game.round.pendingObligations[0] !== "3C"
    ) {
...
check(
  "canStartRearrange: allowed with an outstanding row obligation, blocked with a meld-only (joker) obligation",
  () => {
    const game = freshGameAtPart1();
    game.round.openRow = [card("9", "H")];
    E.drawFromOpenRow(game, "9H");
    ...
    if (!E.canStartRearrange(game))
      throw new Error(
        "should be allowed to start with only the row obligation outstanding",
      );

    game.round.rowObligationCardId = null;
    game.round.pendingObligations = ["SOME_JOKER_ID"];
    if (E.canStartRearrange(game))
      throw new Error("should still be blocked by a meld-only obligation");
  },
);

check(
  "commitRearrange: folding the row-obligated card into a valid group clears the obligation",
  () => {
    const game = freshGameAtPart1();
    game.round.openRow = [card("9", "H")];
    E.drawFromOpenRow(game, "9H");
    ...
    const newGroupId = state1.groups.find((g) =>
      g.cardIds.includes("9H"),
    ).groupId;
    ...
    if (!result.ok)
      throw new Error(
        "commit should succeed: " + JSON.stringify(result.problems),
      );
    if (game.round.pendingObligations.length !== 0)
      throw new Error(
        "folding 9H into a valid run should clear the obligation",
      );
    if (game.round.rowObligationCardId !== null)
      throw new Error("rowObligationCardId should be cleared too");
  },
...
check(
  "commitRearrange: leaving the row-obligated card in hand leaves the obligation outstanding after commit",
  () => {
    const game = freshGameAtPart1();
    game.round.openRow = [card("9", "H")];
    E.drawFromOpenRow(game, "9H");
    ...
    if (
      game.round.pendingObligations.length !== 1 ||
      game.round.pendingObligations[0] !== "9H"
    ) {
      throw new Error(
        "9H should still be an outstanding obligation since it was never placed in a group",
      );
    }
    if (game.round.rowObligationCardId !== "9H")
