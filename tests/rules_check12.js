...
check("newGame: starter is random (both outcomes reachable)", () => {
  ...
  if (gameLow.nextRoundStarter !== 0)
    throw new Error("rng<0.5 should give starter 0");
  if (gameHigh.nextRoundStarter !== 1)
    throw new Error("rng>=0.5 should give starter 1");
});

check(
  "startRound: round 1 uses the game-level starter, current begins there even before Turn 0 resolves",
  () => {
    const game = E.newGame("standard", () => 0.9); // starter = 1
    E.startRound(game, () => 0.5);
    if (game.round.starter !== 1)
      throw new Error(
        "round.starter should match nextRoundStarter, got " +
          game.round.starter,
      );
    if (E.turn0CurrentAskee(game) !== 1)
      throw new Error(
        "Turn 0 should offer the starter (player 1) first, got " +
          E.turn0CurrentAskee(game),
      );
  },
);
...
check("startRound: starter alternates round to round", () => {
  ...
  if (game.round.starter !== 1)
    throw new Error(
      "round 2 starter should alternate to 1, got " + game.round.starter,
    );
  ...
  if (game.round.starter !== 0)
    throw new Error(
      "round 3 starter should alternate back to 0, got " + game.round.starter,
    );
});
