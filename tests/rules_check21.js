...
check(
  "the engine itself does not block further actions once gameOver is true -- that guard lives in app.js",
  () => {
    ...
    if (!game.gameOver || game.winner !== 0)
      throw new Error("test setup: game should be over with player 0 winning");
    ...
    if (game.roundNumber !== 2)
      throw new Error(
        "documenting current behavior: a new round actually starts even though gameOver is true",
      );
  },
);
