...
check(
  "round win via hand-empty discard: winner gets +50 bonus and own meld points, no hand penalty",
  () => {
    ...
    if (!r.ended || r.endReason !== "handout")
      throw new Error("round should end via handout");
    if (r.roundWinner !== 0)
      throw new Error("player 0 should be the winner, got " + r.roundWinner);
    if (r.roundScores[0] !== 65)
      throw new Error(
        "winner score should be 50 bonus + 15 melds = 65, got " +
          r.roundScores[0],
      );
    if (r.roundScores[1] !== 15)
      throw new Error(
        "loser score should be 30 melds - 15 penalty = 15, got " +
          r.roundScores[1],
      );
    if (game.scores[0] !== 65 || game.scores[1] !== 15) {
      throw new Error(
        `game.scores should reflect exactly this round on a fresh game, got [${game.scores}]`,
      );
    }
  },
);
...
check(
  "round win: a mixed-ownership meld attributes points per slot owner, not per meld",
  () => {
    const game = freshGameAtPart2();
    const r = game.round;
    r.tableau.push({
    ...
    if (r.roundScores[0] !== 60)
      throw new Error(
        "player 0 should get bonus(50) + their 2 slots (10) = 60, got " +
          r.roundScores[0],
      );
    if (r.roundScores[1] !== 0)
      throw new Error(
        "player 1 should get their 1 slot (5) - hand penalty (5) = 0, got " +
          r.roundScores[1],
      );
  },
);
...
check(
  "round win: a winner with zero melds still gets exactly the +50 bonus, nothing more",
  () => {
    const game = freshGameAtPart2();
    const r = game.round;
    r.hands[0] = [card("7", "D")];
    ...
    E.discard(game, "7D");
    if (r.roundScores[0] !== 50)
      throw new Error(
        "winner with no melds should score exactly 50, got " + r.roundScores[0],
      );
    if (r.roundScores[1] !== -45)
      throw new Error(
        "loser should be penalized their full hand value, got " +
          r.roundScores[1],
      );
  },
);
