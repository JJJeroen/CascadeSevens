...
check("two jokers each fill a SEPARATE internal gap in the same run", () => {
  ...
  const result = E.autoResolveMeld(hand, [
    "5H",
    "7H",
    "9H",
    "JOKER-1",
  ...
  if (!result.ok || result.type !== "run")
    throw new Error("expected a valid run, got " + JSON.stringify(result));
  if (result.slots.length !== 5)
    throw new Error("expected 5 slots, got " + result.slots.length);
  ...
  const wildRanks = jokerSlots
    .map((s) => s.wildAs.rank)
    .sort()
    .join(",");
  if (wildRanks !== "6,8")
    throw new Error(
      "jokers should fill 6 and 8 respectively, got " + wildRanks,
    );
});

check(
  "maximum legal run: exactly 13 cards (11 reals + 2 jokers filling 2 separate internal gaps)",
  () => {
    // A,2,3,4,5,_,7,_,9,10,J,Q,K -- missing 6 and 8, both internal.
    const reals = ["A", "2", "3", "4", "5", "7", "9", "10", "J", "Q", "K"].map(
      (r) => card(r, "S"),
    );
    const hand = [...reals, joker(1), joker(2)];
    const result = E.autoResolveMeld(
      hand,
      hand.map((c) => c.id),
    );
    if (!result.ok || result.type !== "run")
      throw new Error(
        "a full 13-card run should be valid, got " + JSON.stringify(result),
      );
    if (result.slots.length !== 13)
      throw new Error("expected exactly 13 slots, got " + result.slots.length);
  },
);

check(
  "exceeding the 13-card span is rejected even when the jokers are otherwise unnecessary",
  () => {
    ...
    if (result.ok)
      throw new Error(
        "a 14-card total should be rejected -- runs cap at 13 (a full suit), got " +
