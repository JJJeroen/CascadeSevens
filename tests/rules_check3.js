...
check("undoDraw restores the row and hand, clears the obligation", () => {
  ...
  if (!E.canUndoDraw(game))
    throw new Error("canUndoDraw false right after a row pickup");
  E.undoDraw(game);
  if (game.round.part !== 1) throw new Error("did not return to Part 1");
  if (game.round.openRow.length !== beforeRow)
    throw new Error("row length not restored");
  if (game.round.hands[0].length !== beforeHandLen)
    throw new Error("hand length not restored");
  if (game.round.pendingObligations.length !== 0)
    throw new Error("obligation not cleared");
  if (E.canUndoDraw(game))
    throw new Error("canUndoDraw should be false after undoing");
});
...
check("undoDraw unavailable after any other Part 2 action", () => {
  ...
  E.layNewMeld(game, [{ cardId: "KS" }, { cardId: "KH" }, { cardId: "KD" }]);
  if (E.canUndoDraw(game))
    throw new Error("undo should be closed off after any other Part 2 action");
});

// Joker suit should never be required/stored for sets or runs anymore.
check(
  "joker in a set carries no suit (no false duplicate-card display)",
  () => {
    const game = freshGameAtPart1();
    E.drawFromClosedPile(game);
    const hand = game.round.hands[game.round.current];
    ...
    E.layNewMeld(game, [
      { cardId: "AS" },
      { cardId: "AC" },
      { cardId: "JOKER-1", wildAs: { rank: "A", suit: "S" } },
    ]);
    const meld = game.round.tableau[0];
    const jokerSlot = meld.slots.find((s) => s.card.rank === "JOKER");
    if (jokerSlot.wildAs.suit)
      throw new Error(
        "joker slot still carries a suit: " + JSON.stringify(jokerSlot.wildAs),
      );
  },
);
check("joker fills a run without the caller pre-supplying a suit", () => {
  ...
  const r = E.validateNewMeldSelection(hand, [
    { cardId: "5S" },
    { cardId: "6S" },
    { cardId: "JOKER-1", wildAs: { rank: "7" } },
  ]);
  if (!r.ok)
    throw new Error("run with joker (rank-only wildAs) rejected: " + r.error);
  E.layNewMeld(game, [
    { cardId: "5S" },
    { cardId: "6S" },
    { cardId: "JOKER-1", wildAs: { rank: "7" } },
...
check(
  "addToMeld: joker added to a run needs only rank, suit is implied",
  () => {
    const game = freshGameAtPart1();
    E.drawFromClosedPile(game);
    ...
    if (jokerSlot.wildAs.suit)
      throw new Error("joker slot still carries a suit after addToMeld");
  },
);
check(
  "swapJoker on a run derives suit from the meld, not stored wildAs.suit",
  () => {
    const game = freshGameAtPart1();
    E.drawFromClosedPile(game);
    game.round.comeOut[game.round.current] = true;
    ...
    if (!replacedSlot)
      throw new Error("swap did not place the replacement card");
  },
);
