...
function renderBanner() {
  ...
  if (game.gameOver) {
    ...
    banner.appendChild(
      textEl(
        `${who} won the game! Final: P1 ${game.scores[0]} — P2 ${game.scores[1]}.`,
      ),
    );
...
function renderOpenRow() {
  ...
  const pickable =
    ...
    r.current === 0 &&
    r.part !== "turn0" &&
    CascadeEngine.canDrawFromRow(game);
      ...
      if (canRearrange) {
        cardEl.addEventListener("click", (ev) => {
          ...
          if (slot.card.rank === "JOKER" && selectedHandCardIds.size === 1) {
            const replacement = game.round.hands[0].find(
      ...
      buildCardEl(card, {
        ...
        onClick: () => {
          if (
            r.part === "turn0" &&
            turn0UiMode === "select-swap" &&
            CascadeEngine.turn0CurrentAskee(game) === 0
          ) {
            try {
...
function renderControls() {
  const r = game.round;
  const isHumanTurn =
    !game.gameOver && !r.ended && r.part !== "turn0" && r.current === 0;
  ...
  const comeOutProgress =
    !comeOut && r.comeOutAccum[0] > 0
  ...
  const targetedMeld =
    !rearranging && r.tableau.find((m) => m.id === targetedMeldId);
  ...
  $("drawPileBtn").disabled =
    rearranging || !(isHumanTurn && CascadeEngine.canDrawFromClosedPile(game));
  $("finishDrawingBtn").disabled =
    rearranging || !(isHumanTurn && CascadeEngine.canFinishDrawing(game));
  $("undoDrawBtn").disabled =
    rearranging || !(isHumanTurn && CascadeEngine.canUndoDraw(game));
  ...
  $("startRearrangeBtn").disabled =
    rearranging || !(isHumanTurn && CascadeEngine.canStartRearrange(game));
